...
var DrawerBar=require("./lib/DrawerBar");//搜索页侧边栏
...
var searchScreen=React.createClass({
    ...
    _renderNavigationView: function() {
        return (
            this.state.brandId != null ? <SerialsBar  ref="serialBar" mainNavigator={this.props.mainNavigator} brandId={this.state.brandId}  setSelectedSerial={this.props.setSelectedSerial}/> :
        ...
        return (
            <View style={[styles.flex,styles.container]}>
                ...
                <DrawerBar
                    ...
                    content={parent._renderNavigationView()}>
                    ...
                    this.setState({collectPressed:!this.state.collectPressed});
                        ...
                        <View style={[styles.middel_button,styles.white, styles.row_bottom10]}>
                            {
                                hotBrand.map(function (e, ii) {
                                    var hotBrandkey="hotBrand_"+ii;
                                    return (
                                        <View  key={hotBrandkey} style={[styles.item, styles.flex]}>
                                            <TouchableOpacity style={styles.item} onPress={() => { }}>
...

                        {
                            this.state.brands?this.state.brands.sections.map(function(e,ii){
                                ...
                                var sectionKey="section_"+ii;
                                return (
                                    <View  style={ii==length?{}:styles.row_bottom10} key={sectionKey}>
                                        <View  style={[styles.hotbrand_title,styles.row_bottom1,styles.white]}>
                                            <Text style={{marginLeft:20}}>{e.index}</Text>
                                        </View>
                                        {
                                            e.brands.map(function(eb,ebii){
                                                var brandKey="brand_"+ebii;
                                                return (
                                                    <TouchableOpacity key={brandKey} style={[styles.flex,styles.row_bottom1]} onPress={() => {
                                                           var beforeBrand=parent.state.brandId;
                                                            parent.setState({"brandId":eb.id});
                                                           parent.refs.drawer.openDrawer();

                                                           if(beforeBrand){
                                                             if(beforeBrand!=eb.id)
                                                             parent.refs.serialBar.changeBrand(eb.id);
                                                           }
                    ...
                    <View style={styles.sidebar}>
                        {
                            this.state.brands?this.state.brands.sections.map(function(e,ii){
                                var sideBarKey="sideBar_"+ii;
                                return (
                                    <TouchableOpacity  key={sideBarKey} style={styles.item} onPress={() => {
